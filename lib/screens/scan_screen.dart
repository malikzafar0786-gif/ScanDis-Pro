...
class _ScanScreenState extends State<ScanScreen> {
  ...
  Future<void> _launchScanner() async {
    ...
    try {
      ...
      final paths = result.images ?? const <String>[]; // API returns List<String>?
      if (paths.isEmpty) {
  ...
  /// iOS fallback: google_mlkit_document_scanner has no iOS implementation
  /// (it wraps the Android-only Play Services Document Scanner API), so on
  /// iOS we fall back to repeated camera captures via image_picker. No edge
  /// detection/auto-crop here — swap for a package like
  /// `cunning_document_scanner` if you need that on iOS specifically.
  Future<void> _launchIosCameraLoop() async {
    ...
    try {
      ...
      while (keepGoing) {
        ...
        setState(() {
          _pages.add(ScanPage()
            ..localImagePath = shot.path
            ..order = _pages.length);
        });
        if (!mounted) return;
        keepGoing = await _confirmScanAnotherPage();
      }
    } catch (e) {
  ...
  Future<bool> _confirmScanAnotherPage() async {
    final result = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        backgroundColor: const Color(0xFF14202E),
        title: const Text('Scan another page?', style: TextStyle(color: Colors.white)),
        actions: [
          TextButton(onPressed: () => Navigator.of(ctx).pop(false), child: const Text('Done')),
          FilledButton(onPressed: () => Navigator.of(ctx).pop(true), child: const Text('Next page')),
        ],
      ),
    );
    return result ?? false;
